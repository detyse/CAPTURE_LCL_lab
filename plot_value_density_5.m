...
% tt = compose(s'Color = average(value),  Channel = %s', channel);
title(sprintf('Color = average(value),  Channel = %d', channel));
xlabel('x'); ylabel('y');

% save_path = compose('_pngs_storage', filesep, 'value_density_', channel, '.png');

save_path = fullfile('_pngs_storage', ...
                     compose("value_density_%d.png", channel));
